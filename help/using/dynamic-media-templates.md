...
description: Découvrez comment créer des modèles Dynamic Media à l’aide d’un éditeur de modèles WYSIWYG et inclure plusieurs calques d’images et de texte pour créer rapidement des bannières et des brochures et les utiliser dans des applications en aval.
hide: true
hidefromtoc: true
role: User
...
Créez des modèles Dynamic Media à l’aide d’un éditeur de modèles WYSIWYG et incluez plusieurs images et calques de texte pour créer rapidement des bannières et des brochures et les utiliser dans des applications en aval. Vous pouvez également ajouter des paramètres aux calques d’image et de texte du modèle et utiliser des [URL Dynamic Media](https://experienceleague.adobe.com/fr/docs/commerce-admin/content-design/wysiwyg/storage/catalog-urls-dynamic-media) pour mettre à jour les valeurs de ces calques en temps réel.
...
>[!NOTE]
>
>Les clients et les clientes disposant d’abonnements au SKU de sécurité renforcée ne peuvent pas utiliser les fonctionnalités Dynamic Media, y compris les modèles Dynamic Media, dans ce programme Cloud Services.
...
1. Vérification des éléments suivants sur l’interface utilisateur tactile :
   * Sur la page **[!UICONTROL Modifier la configuration Dynamic Media]**, le **[!UICONTROL Mode de synchronisation de Dynamic Media]**, qui est défini sur **[!UICONTROL Désactivé par défaut]** n’est pas appliqué à tous les dossiers AEM (**[!UICONTROL Synchroniser tout le contenu]** est décoché). Consultez [Configuration de Dynamic Media Cloud Service](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/assets/dynamicmedia/config-dm) pour plus d’informations.
   * Le **[!UICONTROL mode de synchronisation de Dynamic Media]** est défini sur **[!UICONTROL Activer pour les sous-dossiers]** pour le dossier ou le sous-dossier de destination dans lequel vous enregistrerez le modèle après sa création. Consultez [Configuration de Dynamic Media Cloud Service](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/assets/dynamicmedia/config-dm) pour plus d’informations.

## Créer un modèle WYSIWYG Dynamic Media{#how-to-create-dynamic-media-wysiwyg-template}
...
* ![Créer un modèle qui peut être personnalisé instantanément](/help/using/assets/undo.svg) : cliquez pour annuler la dernière action ou appuyez sur **Ctrl** + **Z** (Windows) ou **Cmd** + **Z** (Mac).
...
1. Faites glisser et déposez une image sur la zone de travail pour l’utiliser. Pour redimensionner ou repositionner un calque sur la zone de travail, reportez-vous au [**[!UICONTROL panneau Propriétés]**](#reposition-resize-delete-a-layer).
   ![créer une bannière en un rien de temps](/help/using/assets/add-image-to-canvas.png)
...
1. Dans la liste, sélectionnez le calque.
...
1. Activez le bouton (bascule) **[!UICONTROL Inclure le paramètre]** pour paramétrer une propriété. Voir [ici](#parameterisation-options-or-allowed-parameters) pour connaître le comportement de la propriété après le paramétrage.
...
1. Cliquez sur **[!UICONTROL Enregistrer]**.
   ![création instantanée de contenu](/help/using/assets/parameterise-a-layer.png)Pour basculer entre le panneau Paramètres d’un calque d’image et de texte, sélectionnez le calque sur la zone de travail et cliquez sur **[!UICONTROL Paramètres]**.
...
**X :** permet de déplacer le calque horizontalement le long de son axe central, parallèlement à l’axe X du plan du modèle, en modifiant la valeur du paramètre dans l’URL.
**Y :** permet de déplacer le calque verticalement le long de sa ligne centrale, parallèlement à l’axe Y du plan du modèle, en modifiant la valeur du paramètre dans l’URL.
...
**Masquer :** permet de masquer ou d’afficher le calque dans le modèle à l’aide des valeurs 0 (afficher) et 1 (masquer).
**Source :** permet de remplacer l’image du calque par une nouvelle image en modifiant le chemin d’accès à l’image dans la valeur du paramètre dans l’URL.

**Paramètres de mise en forme de texte :**

Incluez les paramètres ci-dessous pour modifier le texte, sa police, sa couleur et sa taille à partir de l’URL en mettant à jour les valeurs des paramètres dans l’URL.

**Texte :** permet de mettre à jour le texte à partir de l’URL.
**Police de caractères :** permet de mettre à jour la police du texte à partir de l’URL.
**Taille de police :** permet de mettre à jour la taille de police du texte à partir de l’URL.
**Couleur du texte :** permet de mettre à jour la couleur de police du texte à partir de l’URL.

### Regrouper des calques pour contrôler leur visibilité simultanément{#group-layers}

Pour que vos modèles restent flexibles, vous pouvez également utiliser un seul nom de paramètre pour contrôler plusieurs calques. Cette stratégie est utile pour le paramètre de visibilité (masquer ou afficher les calques), afin de mettre à jour la conception ou les images à partir d’un seul modèle.

Pour attribuer le même nom aux paramètres de masquage (![création rapide de contenu](/help/using/assets/Visibility-icon.svg)) de plusieurs calques, afin de masquer ou d’afficher des calques simultanément, procédez comme suit.
...
1. Copiez le nom du paramètre Masquer.
...
1. Effectuez les étapes 3 et 4 de la section [**[!UICONTROL Prévisualiser et publier]**](#preview-and-publish-template-and-copy-template-deliver-url) pour voir vos modifications.
...
1. Cliquez sur **[!UICONTROL Copier l’URL]**. La boîte de dialogue **[!UICONTROL Copier l’URL]** s’affiche. Sélectionnez et copiez l’URL affichée. Notez que le premier paramètre de l’URL commence après un point d’interrogation **(?)** et une paire clé-valeur commence par **$** et se termine par **&amp;**. La clé et la valeur sont séparées par un signe égal **(=)**, avec la clé à gauche et la valeur à droite.
...
* Les sous-chaînes ne sont pas prises en charge dans les calques de texte. Il est impossible d’appliquer différentes propriétés de police à la sous-chaîne d’un calque de texte.
* La prise en charge de plusieurs entreprises pour Dynamic Media n’est actuellement pas disponible avec les modèles Dynamic Media.
* En cas de copie ou de déplacement, le sélecteur de destination affiche tous les dossiers (y compris les dossiers synchronisés autres que Dynamic Media). En outre, à l’heure actuelle, il n’affiche pas les ressources des modèles Dynamic Media (il s’agit de limites du sélecteur de destination).
* Toute opération de mise à jour sur un dossier (par exemple, Publier ou Supprimer) à partir de la section Ressources affecte les modèles Dynamic Media présents dans ce dossier.
* La corbeille ne fonctionne pas pour les modèles Dynamic Media. Si une ressource est déplacée dans la corbeille, puis restaurée, elle est restaurée dans AEM, mais pas dans Dynamic Media. Il en va de même pour les modèles Dynamic Media.

## Voir également

1. Explorer [Dynamic Media et ses fonctionnalités](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/assets/dynamicmedia/dynamic-media)
1. Explorer [Dynamic Media avec les fonctionnalités OpenAPI](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/assets/dynamicmedia/dynamic-media-open-apis/dynamic-media-open-apis-overview)
